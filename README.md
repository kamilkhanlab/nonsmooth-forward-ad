...
- `(y, yDeriv) = eval_gen_derivative(f::Function, x::Vector{Float64}, xDot::Matrix{Float64})`:
	...
	- evaluates `y = f(x)` and a generalized derivative `yDeriv = Df(x; xDot)` according to the nonsmooth vector forward AD mode, analogous to the derivative `Df(x)` computed by the smooth vector forward AD modoe. `xDot` must have full row rank, and defaults to `I` if not provided. `yDeriv` is a `Matrix{Float64}`.
